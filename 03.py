...
def part_1(moves_sets):
    cnt = Counter(points_1(moves_sets[0])) + Counter(points_1(moves_sets[1]))
    reduced_cnt = filter(lambda e: e[1] > 1, cnt.items())
    return min(map(lambda k: abs(k[0][0]) + abs(k[0][1]), reduced_cnt))
    ...
    for move in moves:
        ...
        for _ in range(steps):
            ...
            if direction == 'D':
                ...
                pos[0] += 1
            if (pos[0], pos[1]) not in pts:
                pts[pos[0], pos[1]] = total_steps
    return pts


def part_2(moves_sets):
    pts = [points_2(moves) for moves in moves_sets]
    cnt = Counter(pts[0].keys()) + Counter(pts[1].keys())
    reduced_cnt = filter(lambda e: e[1] > 1, cnt.items())
    return min(map(lambda k: pts[0][k[0]] + pts[1][k[0]], reduced_cnt))


print('Part 1:', part_1(in_test1))
print('Part 1:', part_1(in_test2))
print('Part 1:', part_1(in_main))
print('Part 2:', part_2(in_test1))
print('Part 2:', part_2(in_test2))
